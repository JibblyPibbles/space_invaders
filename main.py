...
def run_game():
    ...
    while game_on:
        ...
        random_chance = randint(1, 60)
        random_ship = choice(ship_manager.all_ships)
        for laser in alien_laser_manager.all_lasers:
            if laser.tracking == True:
                if random_ship.type == 'elite' and random_chance < 20:
                    laser.goto(random_ship.xcor(), random_ship.ycor())
                    laser.tracking = False
                elif random_ship.type == 'veteran' and random_chance < 10:
                    laser.goto(random_ship.xcor(), random_ship.ycor())
                    laser.tracking = False
